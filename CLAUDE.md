# Claude Code project notes — unifi_network_application

Fork of [https://github.com/tigattack/ansible-role-unifi-network-application](https://github.com/tigattack/ansible-role-unifi-network-application)
by [tigattack](https://github.com/tigattack). Full credit to tigattack for the original design and implementation.
This fork is maintained at [https://github.com/basictheprogram/ansible-role-unifi-network-application](https://github.com/basictheprogram/ansible-role-unifi-network-application).

Issues should be opened against this fork. If a fix is applicable to the upstream role,
a pull request will be submitted to tigattack's repository.

Deploys the UniFi Network application and a companion MongoDB instance as Docker containers using the `linuxserver/unifi-network-application` image. Optionally deploys a `mongo-express` web UI. Manages Docker network creation, directory layout, MongoDB init script, container lifecycle, and log rotation.
...
Role-specific subsystem scopes: `containers`, `mongo`, `network`
...
1. `tasks/main.yml` — core orchestration (user/group lookup, directory
   creation, Docker network, container deploy loop); already implemented.
2. `files/mongo-init.sh` — MongoDB user creation init script; already
   implemented.
3. `handlers/main.yml` — Docker image prune handler; already implemented.
4. `defaults/main.yml` + `vars/main.yml` — all role variables and computed
   container definitions; already implemented.
5. `meta/argument_specs.yml` — full variable documentation; already
   implemented.
6. `tasks/preflight.yml` — fail-fast assertions (Ansible version, required
   secrets); not yet implemented.
7. Molecule test suite — verify idempotency and container health; partially
   implemented (converge + prepare exist, no verify playbook or testinfra
   tests yet).
...
* Two variables are **required** (no defaults): `unifi_network_application_mongo_password`
  and `unifi_network_application_mongo_root_password`. The play will fail
  without them.
* The Docker network named by `unifi_network_application_docker_network`
  is created by the role if absent — callers do not need to pre-create it.
...
for feature-driven changes (e.g., `feat(mongo): ...`) and a role-layout
...
* Renames or removes a handler, tag, or public task file name
...
Examples:
    ...
    feat(containers): add mongo-express optional container
    fix(mongo): correct init script db grant for unifi_audit
    refactor(tasks): split install and configure into files
    chore(meta): bump minimum Ansible version to 2.20
    test(molecule): add verify playbook with testinfra
